...
class PBRTextureProcessor:
    ...
    def generate_roughness_map(self, diffuse_path: str, output_path: str) -> None:
        ...
        # Enhance contrast to emphasize skin pores
        ...
        # Normalize to 0.4-0.6 range for skin
        arr = np.array(roughness, dtype=np.float32) / 255.0
        arr = 0.4 + (arr * 0.2)  # Map to [0.4, 0.6]
    ...
    def generate_metallic_map(self, output_path: str, default_value: float = 0.0) -> None:
        ...
        # Skin texture size (assume 2048x2048)
        size = (2048, 2048)
        
        # Create mostly black texture (no metallic)
    ...
    def generate_ao_map(self, diffuse_path: str, normal_path: str, 
        ...
        """
        Generate Ambient Occlusion map from normal + diffuse.
        Darkens crevices (eye sockets, mouth corners, skin folds).
        """
        logger.info(f"Generating AO map: {output_path}")
        
        # Load base textures
        diffuse = self.load_texture(diffuse_path)
        normal_img = Image.open(normal_path)
        
        # Simplified AO: Use high-pass filter on diffuse
        diffuse_pil = Image.open(diffuse_path).convert('L')
        
        # High-pass filter to find crevices
        blurred = diffuse_pil.filter(ImageFilter.GaussianBlur(radius=5))
        
        # Subtract blurred from original (high-pass)
