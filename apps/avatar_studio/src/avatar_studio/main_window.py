...
BackgroundOperation = Callable[[ProgressSink], object]
...
class OperationWorker(QObject):
    ...
    def __init__(self, operation: BackgroundOperation) -> None:
        super().__init__()
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        product_name = QLabel("AVATAR STUDIO")
        product_name.setObjectName("productName")
        product_tagline = QLabel("Digital human production workspace")
        product_tagline.setObjectName("productTagline")
        identity.addWidget(product_name)
        identity.addWidget(product_tagline)
        app_header_layout.addLayout(identity, stretch=1)
        header = QHBoxLayout()
        self.project_label = QLabel()
        self.project_label.setObjectName("statusBadge")
        self.progress_bar = QProgressBar()
        self.progress_bar.setRange(0, 1000)
        self.progress_bar.setFixedWidth(260)
        header.addWidget(self.project_label)
        header.addWidget(self.progress_bar)
        app_header_layout.addLayout(header)
        ...
        left_layout.addWidget(pipeline_title)
        ...
        self.operation_progress.setFormat("No operation running")
        ...
        artifacts_title = QLabel("STAGE ARTEFACTS")
        artifacts_title.setObjectName("sectionTitle")
        right_layout.addWidget(artifacts_title)
        ...
        add_artifact.setObjectName("primaryButton")
        ...
        parameters_title = QLabel("TECHNICAL PARAMETERS")
        parameters_title.setObjectName("sectionTitle")
        right_layout.addWidget(parameters_title)
        ...
        self.log.setToolTip("Operation output and local tool diagnostics")
    ...
    def _operation_text(self, stage_id: str) -> tuple[str, bool]:
        ...
        if stage_id == "03-reconstruction":
            return ("COLMAP dense reconstruction with phase-aware progress: undistortion, PatchMatch stereo, fusion and meshing.", True)
        if stage_id == "19-piper-integration":
            return ("Generate speech with Piper or normalize audio with FFmpeg. Output and provenance are registered automatically.", True)
        if stage_id.startswith(tuple(f"{n:02d}-" for n in range(4, 19))):
            return ("For Blender-based manual stages, Avatar Studio performs deep scene inspection and records the report. Editing remains in Blender.", True)
    ...
    def _pass_current_stage(self) -> None:
        ...
        if not allowed:
            ...
            answer = QMessageBox.question(self, "Definition of Done not satisfied", f"This stage cannot pass normally:\n{reason_text}\n\nRecord a controlled waiver?")
            if answer != QMessageBox.Yes:
                return
            waiver, ok = QInputDialog.getMultiLineText(self, "DoD waiver", "Explain why proceeding is justified, what risk remains and how it will be verified later:")
    ...
    def _run_stage_operation(self) -> None:
        ...
        if stage_id == "01-reference-acquisition":
            ...
            if not ok:
                return
            operation = lambda progress: self.operations.colmap_sparse(images, matcher=matcher, progress_callback=progress)
            ...
            if not ok:
                return
            operation = lambda progress: self.operations.colmap_dense(images, sparse, mesher=mesher, progress_callback=progress)
            ...
            if mode.startswith("Piper"):
                ...
                if not output:
                    return
                operation = lambda progress: self.operations.synthesize_piper(text, model, output, progress_callback=progress)
            else:
                ...
                if not output:
                    return
                operation = lambda progress: self.operations.normalize_audio(source, output, progress_callback=progress)
            ...
            if not blend:
                return
            operation = lambda progress: self.operations.inspect_blend(stage_id, blend, progress_callback=progress)
    ...
    def _start_background_operation(self, stage_id: str, operation: BackgroundOperation) -> None:
        ...
        self.operation_progress.setValue(0)
        self.operation_progress.setFormat("Starting… 0%")
    ...
    def _operation_progress(self, value: int, message: str) -> None:
        ...
        self.operation_progress.setFormat(f"{message} — {value}%")
        self.log.appendPlainText(f"{self.pending_stage_id or 'operation'}: {value}% {message}")
    ...
    def _operation_failed(self, message: str) -> None:
        ...
        self.operation_progress.setFormat("Operation failed")
        if stage_id:
