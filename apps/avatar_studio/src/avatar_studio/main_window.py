...
from avatar_studio.preview import ArtifactPreview
from avatar_studio.reporting import write_project_report
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        right_layout.addWidget(self.artifact_preview, stretch=1)
        ...
        self.artifact_details.setMaximumHeight(220)
        right_layout.addWidget(self.artifact_details)
        splitter.addWidget(right)
        splitter.setSizes([320, 690, 570])
        ...
        project_menu.addAction("Open / create workspace…", self._switch_workspace)
        project_menu.addAction("Generate final report", self._generate_project_report)
    ...
    def _switch_workspace(self) -> None:
        selected = QFileDialog.getExistingDirectory(
            self, "Select or create Avatar Studio workspace"
        )
        if not selected:
            return
    ...
    def _generate_project_report(self) -> None:
        try:
            markdown_path, json_path = write_project_report(self.store)
    ...
    def _refresh_stage_list(self) -> None:
        ...
        for stage in STAGES:
            ...
            item = QListWidgetItem(
                f"{marker} {stage.order:02d}  {stage.title}  [{status}]"
            )
    ...
    def _operation_text(self, stage_id: str) -> tuple[str, bool]:
        if stage_id == "02-photogrammetry":
            return (
                "COLMAP sparse reconstruction: feature extraction, matching and camera "
                "registration. You select the photo folder and parameters; no CLI is required.",
                True,
            )
        if stage_id == "03-reconstruction":
            return (
                "COLMAP dense reconstruction: undistortion, PatchMatch stereo, fusion and "
                "Poisson/Delaunay meshing.",
                True,
            )
        if stage_id == "19-piper-integration":
            return (
                "Generate speech with Piper or normalize audio with FFmpeg. The generated "
                "output and provenance report are registered in the project.",
                True,
            )
        if stage_id.startswith(tuple(f"{n:02d}-" for n in range(4, 19))):
            return (
                "For Blender-based manual stages, Avatar Studio can currently perform deep "
                "inspection of a selected .blend scene and record the report. Editing remains "
                "in Blender.",
                True,
            )
        return (
            "No automated production operation is implemented for this stage yet. Follow the "
            "linked documentation and register the resulting artefact.",
            False,
        )
    ...
    def _update_current_view(self) -> None:
        ...
        outputs = (
            "".join(f"<li>{value}</li>" for value in stage.expected_outputs)
            or "<li>stage report</li>"
        )
        ...
        self.stage_details.setHtml(
            ...
            f"<p><b>Validation:</b> {len(validations)} result(s), {failed} failed, "
            f"{waivers} waiver(s).</p>"
    ...
    def _pass_current_stage(self) -> None:
        ...
        if not allowed:
            ...
            waiver, ok = QInputDialog.getMultiLineText(
                self,
                "DoD waiver",
                "Explain why proceeding is justified, what risk remains and how it will be "
                "verified later:",
            )
            ...
            if not self.store.artifacts_for_stage(stage_id):
                QMessageBox.warning(
                    self,
                    "Artefact still required",
                    "A waiver cannot replace the stage output artefact. Register an artefact "
                    "before passing.",
                )
                return
        allowed, reasons = self.store.can_pass_stage(stage_id)
    ...
    def _register_artifact(self) -> None:
        ...
        path, _ = QFileDialog.getOpenFileName(
            self, "Register stage artefact", str(self.store.workspace)
        )
        if not path:
            return
        try:
    ...
    def _probe_tools(self) -> None:
        ...
        for probe in probe_default_tools(configured):
            ...
            self.log.appendPlainText(
                f"  [{state}] {probe.name}: {display_path(probe.executable)}"
            )
    ...
    def _run_stage_operation(self) -> None:
        ...
        if stage_id == "02-photogrammetry":
            images = QFileDialog.getExistingDirectory(
                self, "Select source photographs", str(self.store.workspace)
            )
            if not images:
                return
            matcher, ok = QInputDialog.getItem(
                self,
                "COLMAP matcher",
                "Matching strategy",
            ...
            if not images:
                return
            sparse = QFileDialog.getExistingDirectory(
                self,
                "Select COLMAP sparse model (usually sparse/0)",
                str(self.store.workspace / "work" / "colmap"),
            )
            if not sparse:
                return
            mesher, ok = QInputDialog.getItem(
                self,
                "Mesher",
                "Mesh reconstruction",
                ["poisson", "delaunay"],
            ...
            mode, ok = QInputDialog.getItem(
                ...
                "Speech operation",
                "Choose operation",
                ["Piper synthesis", "FFmpeg normalize WAV"],
                0,
                False,
            )
            if not ok:
            ...
            if mode.startswith("Piper"):
                text, ok = QInputDialog.getMultiLineText(
                    self, "Speech text", "Text to synthesize"
                )
                if not ok or not text.strip():
                    return
                model, _ = QFileDialog.getOpenFileName(
                    self,
                    "Select Piper ONNX model",
                ...
                output, _ = QFileDialog.getSaveFileName(
                    self,
                    "Output WAV",
                    str(self.store.workspace / "speech.wav"),
                    "WAV (*.wav)",
                )
                if not output:
                    return
                ...
                source, _ = QFileDialog.getOpenFileName(
                    self, "Select audio", str(self.store.workspace)
                )
                if not source:
                    return
                output, _ = QFileDialog.getSaveFileName(
                    self,
                    "Normalized WAV",
                    str(self.store.workspace / "speech_normalized.wav"),
                    "WAV (*.wav)",
                )
                if not output:
                    return
            ...
            blend, _ = QFileDialog.getOpenFileName(
                self,
                "Select Blender scene to inspect",
                str(self.store.workspace),
                "Blender (*.blend)",
            )
            if not blend:
    ...
    def _start_background_operation(
        self, stage_id: str, operation: Callable[[], object]
    ) -> None:
    ...
    def _cancel_operation(self) -> None:
        ...
        self.log.appendPlainText(
            "Cancellation requested. The active external process will be terminated."
        )
        self.cancel_button.setEnabled(False)
    ...
    def _operation_finished(self, result: object) -> None:
        ...
        self.log.appendPlainText(
            f"{stage_id}: operation completed; report: {report_path}"
        )
        if stage_id == "02-photogrammetry":
            ...
            if models:
                self.store.add_validation_result(
                    stage_id,
                    "sparse_model_created",
                    "passed",
                    value=len(models),
                    expected=">= 1",
                    message="COLMAP created at least one sparse model.",
                )
            else:
                self.store.add_validation_result(
                    stage_id,
                    ...
                    value=0,
                    expected=">= 1",
                    message="No sparse reconstruction model was created.",
                )
        elif stage_id == "03-reconstruction":
            mesh = Path(report["mesh"])
            self.store.register_artifact(
                stage_id, mesh, metadata={"operation_report": str(report_path)}
            )
            self.store.add_validation_result(
                stage_id,
                "dense_mesh_created",
                "passed",
                value=mesh.stat().st_size,
                expected="> 0 bytes",
                message="Dense reconstruction produced a mesh.",
            )
        elif stage_id == "19-piper-integration":
            ...
            if output and Path(output).is_file():
                self.store.register_artifact(
                    stage_id,
                    output,
                    metadata={"operation_report": str(report_path)},
                )
            else:
                self.store.register_artifact(
                    stage_id, report_path, kind="operation_report"
                )
            self.store.add_validation_result(
                stage_id,
                "speech_operation",
                "passed",
                message="Speech processing operation completed successfully.",
            )
        else:
            self.store.register_artifact(
                stage_id, report_path, kind="inspection_report"
            )
            self.store.add_validation_result(
                stage_id,
                "blender_inspection",
                "passed",
                message="Blender scene inspection completed successfully.",
            )
        self._refresh_stage_list()
    ...
    def _operation_failed(self, message: str) -> None:
        ...
        if stage_id:
            self.store.add_validation_result(
                stage_id, "tool_operation", "failed", message=message
            )
